export default {
  ...
  chat: {
    ...
    openHistory: '打开历史',
    hermesHistory: 'AUD 历史',
    historyScopeHint: '这里按来源只读查看 AUD 历史会话。',
    noSessions: '暂无会话',
    searchTitle: '搜索会话',
    searchSubtitle: '按标题或消息内容搜索',
    searchScope: '搜索范围：仅 Web UI 本地会话库；不包含只读 AUD 历史会话。',
  ...
  plugins: {
    title: '插件',
    refresh: '刷新',
    notice: '只读展示可发现的 AUD 插件 manifest。发现元数据读取不会加载插件代码。v1 管理动作仍保留在 CLI，新 AUD 会话生效。',
    loadFailed: '加载插件失败',
    commandCopied: '命令已复制',
  ...
  models: {
    ...
    copilotAddDetected: '已在本机检测到 GitHub Copilot OAuth 凭证，点击「添加」即可在 AUD 中启用 Copilot。',
    ...
    copilotDeleteHintGhCli: 'Copilot 将从 AUD 列表移除。不会影响 gh CLI —— `gh auth status` 仍显示已登录。',
    copilotDeleteHintAppsJson: 'Copilot 将从 AUD 列表移除。不会影响 VS Code Copilot 插件的登录。',
    ...
    aliasHint: '仅修改 Web UI 显示名，发送给 AUD 的仍是原始模型 ID。',
    ...
    visibilityHint: '仅影响 Web UI 的模型选择器和模型页展示，不会改写 AUD CLI 的 provider/model 配置。实际调用仍使用原始模型 ID。',
  ...
  changelog: {
    ...
    new_0_5_15_9: '修复 AUD Markdown 媒体渲染与同步重试',
    ...
    new_0_5_22_1: '修复 AUD 会话列表接口错误读取 Web UI 本地会话库的问题',
    ...
    new_0_5_26_6: '隔离 Bridge Profile 环境，并修复不同 Python 环境下的 AUD 插件发现',
    ...
    new_0_5_26_10: '支持未找到源码目录时使用包安装的 AUD',
    ...
    new_0_5_5_2: '新增历史页面，用于浏览 AUD 会话历史记录',
    ...
    new_0_5_4_3: '修复 Docker 部署后 Web 终端无法使用 AUD CLI 的问题',
    new_0_5_4_4: '添加工作区对话框标题 i18n 翻译，改进会话持久化',
    new_0_5_4_5: '支持代码块复制反馈，显示用户通知',
    new_0_5_4_6: '对齐使用分析与 AUD 状态数据库架构',
